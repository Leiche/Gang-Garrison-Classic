...
//*************************************
if (victim.class == CLASS_CIVILIAN) { // For GGC V.I.P.
    if (global.isHost = true) {
        sendEventEscortKilled(killer);
        doEventEscortKilled(killer);
    }
}
victim.deaths += 1;
...
if(victim.object.fussball) {
    if(killer != -1 && killer != victim) {
        recordEventInLog(12, victim.team, victim.name);
        recordEventInLog(13, killer.team, killer.name);
    }
}
...
randomize();
with(victim.object) {
    if((damageSource == WEAPON_ROCKETLAUNCHER or damageSource == WEAPON_QROCKETLAUNCHER or damageSource == WEAPON_MINEGUN or damageSource == FRAG_BOX or damageSource == WEAPON_REFLECTED_STICKY or damageSource == WEAPON_REFLECTED_ROCKET) && (global.gibLevel>1) && distance_to_point(xoffset+xsize/2,yoffset+ysize/2) < 900) {
